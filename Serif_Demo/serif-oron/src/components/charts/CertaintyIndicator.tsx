...
const getCertaintyLevel = (value: number) => {
  ...
  if (normalized >= 85) return { label: 'Very High', color: 'green', description: 'Strong personal evidence — high confidence' }
  if (normalized >= 70) return { label: 'High', color: 'primary', description: 'Good personal evidence supports this insight' }
  if (normalized >= 50) return { label: 'Moderate', color: 'blue', description: 'Mixed evidence — personal data partially supports' }
  if (normalized >= 30) return { label: 'Low', color: 'amber', description: 'Limited personal data — mostly population-based' }
  return { label: 'Exploratory', color: 'gray', description: 'Preliminary — based mostly on population data' }
}
      ...
      return (
        <div ref={ref} className={cn('inline-flex items-center gap-1.5', className)} {...props}>
          ...
          >
            ...
            {showLabel && <span className="ml-0.5 opacity-75">{level.label}</span>}
          </span>
          {showTooltip && (
