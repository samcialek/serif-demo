  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <header
        ref={ref}
        className={cn(
          'h-16 bg-gradient-to-r from-white to-slate-50/80 border-b border-slate-200 flex items-center justify-between px-6',
          className
        )}
        {...props}
      >
        {/* Left side - Static identity block */}
        <div className="flex items-center gap-4">
          <div data-tour="persona-selector">
            ...
            <span className="text-sm font-medium text-slate-700">Oron A.</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Tooltip content={`Last synced: ${timeSinceSync}`}>
            ...
            >
              <RefreshCw className={cn('w-4 h-4 transition-transform', isSyncing && 'animate-spin')} />
          ...
          <button className="relative p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-lg transition-colors">
            ...
            <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-serif-pink rounded-full">
              <span className="absolute inset-0 rounded-full bg-serif-pink animate-ping opacity-40" />
            </span>
          </button>
