...
class QuizEngine {
  ...
  getNextQuestion(): QuestionDef | null {
    if (this.fixedPhase) {
      if (this.fixedIndex < FIXED_12.length) {
        const qid = FIXED_12[this.fixedIndex]!;
        return BANK_BY_ID.get(qid) ?? null;
      }
      // Finished fixed phase — compute posteriors
    ...
    return selectNextQuestion(this.state, QUESTION_BANK, ARCHETYPES);
  }
